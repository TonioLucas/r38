...
class BTCPayService:
    ...
    async def create_invoice(
        ...
        Returns:
            Dictionary with invoice details (id, checkoutLink, etc.)
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info(
                    f"Created BTCPay invoice: {invoice.get('id')} for subscription {subscription_id}"
                )

                return {
                    "invoiceId": invoice["id"],
                    "checkoutLink": invoice["checkoutLink"],
                    "btcAmount": invoice.get("amount", str(amount)),
                }

        except httpx.HTTPStatusError as e:
            logger.error(
                f"BTCPay API error creating invoice: {e.response.status_code} - {e.response.text}"
            )
            ...
            logger.error(
                f"BTCPay API timeout creating invoice for subscription {subscription_id}"
            )
            raise ExternalServiceError(
                service="BTCPayServer",
            ...
            logger.error(
                f"Unexpected error creating BTCPay invoice: {e}", exc_info=True
            )
            raise ExternalServiceError(
                service="BTCPayServer",
    ...
    def verify_webhook_signature(self, payload: str, signature: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error verifying BTCPay webhook signature: {e}", exc_info=True
            )
            return False
    ...
    async def get_invoice(self, invoice_id: str) -> dict:
        ...
        try:
            ...
            logger.error(
                f"BTCPay API error retrieving invoice: {e.response.status_code} - {e.response.text}"
            )
            raise ExternalServiceError(
                service="BTCPayServer",
